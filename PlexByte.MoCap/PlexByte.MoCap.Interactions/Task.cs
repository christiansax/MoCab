...
namespace PlexByte.MoCap.Interactions
{
        ...
        ITask
    {
        ...
        public void OnStateChanged(InteractionEventArgs pEventArgs) { StateChanged?.Invoke(this, pEventArgs); }
        ...
        public virtual void ChangeIsActive(bool pActive)
        {
            if (_state == InteractionState.Active || _state == InteractionState.Queued)
            {
                if (IsActive != pActive)
                {
                    ...
                    OnModify(new InteractionEventArgs("IsActive state changed",
                        DateTime.Now,
                        InteractionType.Task,
                        changedAttrs));
                }
        ...
        public virtual void AddTimeslice(int pDuration)
        {
            ...
            OnModify(new InteractionEventArgs("Time used changed",
                DateTime.Now,
                InteractionType.Task,
                changedAttrs));
        }
        ...
        public virtual void AddExpense(decimal pExpenseValue)
        {
            ...
            OnModify(new InteractionEventArgs("Budget used changed",
                DateTime.Now,
                InteractionType.Task,
                changedAttrs));
        }
        ...
        public void UdateProgress(int pProgress)
        {
            if (pProgress > 0)
            {
                _progress = ((pProgress + _progress) >= 100)
                    ? 100
                    : _progress + pProgress;
                ...
                else
                {
                    ...
                    OnProgressUpdated(new InteractionEventArgs($"Progress changed [NewValue={_progress}] [TaskId={_id}]",
                        DateTime.Now,
                        InteractionType.Task,
                        changedAttributes));
                }
